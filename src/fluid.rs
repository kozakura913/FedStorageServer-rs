...
use tokio::{io::{AsyncRead, AsyncReadExt, AsyncWrite, AsyncWriteExt}, sync::RwLock};
...
impl FluidId{
	fn new(mut name:String,nbt:Option<impl AsRef<[u8]>>)->Self{
		if let Some(nbt)=nbt{
			...
			let result = result.iter().map(|n| format!("{:02X}", n)).collect::<String>();
			name+=&result;
		}
		Self(name)
	}
...
impl FluidStack{
	...
	async fn write<W:AsyncWrite + std::marker::Unpin>(&self,w:&mut W)-> Result<(),tokio::io::Error>{
		...
		match self.nbt.as_ref(){
			...
			Some(nbt)=>{
				let len=nbt.len().try_into().map_err(|e|tokio::io::Error::other(e))?;
...
impl ClientSession{
	pub(crate) async fn fluid_recv(&mut self) -> Result<(),tokio::io::Error>{
		...
		let cache_hit={
			self.go.fluid_buffers.read().await.get(self.freq()).cloned()
		};
	...
	pub(crate) async fn fluid_send(&mut self) -> Result<(),tokio::io::Error>{
		let fs=FluidStack::read(&mut self.reader).await?;
		let freq_buffer=if let Some(cache_hit)=self.go.fluid_buffers.read().await.get(self.freq()){
			cache_hit.clone()
		}else{
...
mod tests {
	...
	fn update_fluids(){
		tokio::runtime::Builder::new_current_thread().enable_all().build().unwrap().block_on(async{
			...
			assert_eq!(fluids.take_fluid(FluidStack::dummy()).await,Some(FluidStack::dummy()));
			assert_eq!(fluids.data.read().await.values().next().cloned(),Some(FluidStack::dummy()));
			assert_eq!(fluids.take_fluid(FluidStack::any()).await,Some(FluidStack::dummy()));
			fluids.insert_fluid(FluidStack::dummy()).await;
			fluids.insert_fluid(FluidStack::dummy()).await;
			assert_ne!(fluids.data.read().await.values().next().cloned(),Some(FluidStack::dummy()));
		});
	}
	#[test]
	fn read_write_fluid(){
		...
		tokio::runtime::Builder::new_current_thread().enable_all().build().unwrap().block_on(async{
			...
			let dst=FluidStack::read(&mut std::io::Cursor::new(&v)).await.unwrap();
			assert_eq!(src,dst);
		});
	}
}
