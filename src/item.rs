...
impl Items{
	...
	async fn insert_items(&self,stacks:&mut Vec<ItemStack>){
		let mut data=self.data.write().await;
		let max_stacks=stacks.len().min(ITEM_BUFFER_LIMIT.saturating_sub(data.len()));
...
impl ClientSession{
	pub(crate) async fn item_recv(&mut self) -> Result<(),tokio::io::Error>{
		...
		let data_size=data_size.try_into().map_err(|e|tokio::io::Error::other(e))?;
...
#[cfg(test)]
mod tests {
    use super::{Items,ItemStack,ITEM_BUFFER_LIMIT};
	...
	fn read_write_item(){
		...
		tokio::runtime::Builder::new_current_thread().enable_all().build().unwrap().block_on(async{
			...
			let dst=ItemStack::read(&mut std::io::Cursor::new(&v)).await.unwrap();
			assert_eq!(src,dst);
		});
	}
}
